    ...
    this.dialog
      ...
      .subscribe((result) => {
        if (result.event == "inviteUser") {
          result.data.invitedUserResponses.forEach((invitedUserResponse) => {
            if (invitedUserResponse.IsInviteSuccessful) {
              this.dataSource.data.push(invitedUserResponse.invitedUser);
            }
            if (invitedUserResponse.message) {
              //toster
            }
          });
          //refresh
      ...
      .forEach((user) => {
        ...
        this.userService.updateUserStatus(userStatus).subscribe((_res: any) => {
          if (_res) {
            ...
            //<toster>: user status updated successfully
          }
        });
      });
  }
      ...
      .subscribe((res) => {
        if (res) {
          this.userService.deleteUser(row.userId).subscribe((data) => {
            ...
            //<toster>: user deletion: user deleted successfully
          });
        }
      });
  }
